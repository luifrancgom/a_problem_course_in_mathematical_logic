...
- To find the maximum value of $p(\alpha) / \ell(\alpha)$ it is important to point out that:
    ...
    + To maximize $p(\alpha) / \ell(\alpha)$ we need to minimize $\ell(\alpha)$ by choosing the minimum value of $a(\alpha)$
        ...
        + The minimum for $a(\alpha)$ is 1 and corresponds to formulas corresponds to formulas built from negating a formula at each step and starting with an atomic formula: $A_0, (\neg A_0), (\neg (\neg A_0)), \dots$
